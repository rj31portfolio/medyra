...
$service = trim((string) ($_POST['services'] ?? ''));
$date = trim((string) ($_POST['date'] ?? ''));

if ($firstName === '' || $lastName === '' || $phone === '' || $email === '' || $service === '' || $date === '') {
    http_response_code(422);
    echo json_encode(['status' => 'error', 'message' => 'Please fill in all required appointment form fields.']);
    exit;
}
...
$subject = 'New Appointment Request - Medyra Website';
$plainBody = implode("\n", [
...
$htmlBody = '<h2>New Appointment Request</h2>'
    ...
    . '<p><strong>Preferred Date:</strong> ' . htmlspecialchars($date, ENT_QUOTES, 'UTF-8') . '</p>';
